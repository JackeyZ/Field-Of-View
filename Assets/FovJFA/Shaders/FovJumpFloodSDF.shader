	...
	SubShader 
	{
		...
		Pass
		{
			...
			float4 frag (fragInput input) : SV_Target
			{
				...
				float4 value = _MapDepthTexture.Sample(_SDFTexture_Trilinear_Clamp_Sampler, uv);
			...
			float4 frag (fragInput input) : SV_Target
			{
			    ...
			    for (int y = -1; y <= 1; ++y) 
			    {
        			for (int x = -1; x <= 1; ++x) 
        			{
						...
						// only need RG
						if ((seedCoord.x != 0.0 || seedCoord.y != 0.0) && distance < minDistance) // 如果有阻挡物并且距离是最小距离则把坐标储存下来
			            {
